  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', width: 720, maxWidth: '95vw', background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 28, zIndex: 1, boxShadow: '0 24px 80px rgba(0,0,0,0.6)', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: C.textSec, margin: 0 }}>
              Frequency of failure events by Remaining Useful Life window — last {rulToolChangeCount} tool changes
            </p>
          </div>
        ...
        {/* Stats summary row — driven by rulSummaryStats from data.ts */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          {rulSummaryStats.map((s, i) => (
